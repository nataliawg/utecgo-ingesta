...
#logs
log_file = '/app/logs/ms_estudiantes.log' #cambiar aca
log_format = '%(asctime)s %(levelname)s %(name)s %(message)s'

logging.basicConfig(level=logging.INFO, format=log_format, filename=log_file, filemode='a')
logger = logging.getLogger('ingesta.estudiantes') #cambiar aca
...
def get_dynamodb_table(region_name):
    table_name = 'tabla_estudiantes'  #cambiar aca
...
def main():
    ...
    output_file = f'/app/ingesta/{stage}_estudiantes_data.json' #cambiar aca
    save_to_file(estudiantes, output_file)
    
    s3_key = f'estudiantes/{stage}_estudiantes_data.json' #cambiar aca
