...
#logs
log_file = '/app/logs/ms_boletas.log' #cambiar aca
log_format = '%(asctime)s %(levelname)s %(name)s %(message)s'

logging.basicConfig(level=logging.INFO, format=log_format, filename=log_file, filemode='a')
logger = logging.getLogger('ingesta.boletas') #cambiar aca
...
def get_dynamodb_table(region_name):
    table_name = 'tabla_boletas'  #cambiar aca
...
def main():
    ...
    output_file = f'/app/ingesta/{stage}_boletas_data.json' #cambiar aca
    save_to_file(estudiantes, output_file)
    
    s3_key = f'boletas/{stage}_boletas_data.json' #cambiar aca
